			...
			try {
				...
				synchronized (AppConfig.colorLock) {
					...
					if(clientMessage.getMessageType() == MessageType.LY_MARKER) {				
						...
						// Ako smo pronasli razliku 
						if (initiatorDoingSnapshot != -1) {
							
								// Zamenjujemo lokalnu initiatorsVersion sa novom koju smo dobili u LY_MARKERU
								...
								int collectorID = Integer.parseInt(clientMessage.getMessageText());			// serventID serventa koji je zapoceo Snapshot
								
								lyBitcakeManager.markerEvent(collectorID, snapshotCollector);
						}
					}
				}
